...
{
    [Header("Browken Wall Reference")]
    public Transform BrowkenWall;
   
    
    [Space, Header("Effects Reference")]
    ...
    void Explode(Transform target)
    {
        ...
        for (int i = 0; i < triangles.Length; i += 3)
        {
            ...
            float x = Random.Range(target.parent.position.x - target.parent.localScale.x, target.parent.position.x + target.parent.localScale.x) / 2;
            float y = target.parent.position.y;
            float z = Random.Range(target.parent.position.z, target.parent.position.z + Random.Range (0.0f, 0.3f));
            Vector3 pos = new Vector3(x, y, z);

            CreateMeshPiece(extrudeSize, pos, target.GetComponent<Renderer>().material, index, averageNormal, vertices[triangles[i]], vertices[triangles[i + 1]], vertices[triangles[i + 2]], uvs[triangles[i]], uvs[triangles[i + 1]], uvs[triangles[i + 2]]);
    ...
    void CreateMeshPiece(float extrudeSize, Vector3 pos, Material mat, int index, Vector3 faceNormal, Vector3 v1, Vector3 v2, Vector3 v3, Vector2 uv1, Vector2 uv2, Vector2 uv3)
    {
        ...
        float ScaleFactor = (GameManager.instance.AraSize / GameManager.instance.ui.AreaResizer.maxValue);
